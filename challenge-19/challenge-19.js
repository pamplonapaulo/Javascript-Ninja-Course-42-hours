(function(){
    ...
    */
    ...
    correspondências de qualquer string, não somente para a primeira, ainda que esta tenha muitas linhas, deve sempre casar com números no início de cada linha, independente de quantos caracteres de número estiverem juntos.
    ...
    */
    ...
    - Essa função receberá dois parâmetros: o primeiro chamado `markup`, que será a marcação HTML testada, e o segundo `cssClass`, que será a classe que iremos testar;
    ...
    - Lembrando que a função deve funcionar para qualquer marcação HTML e para qualquer classe que for testada. Os dados passados no exercício são somente para exemplificar.
    */
    ...
    function hasClass(markup, cssClass){
        ...
        var regex = new RegExp('(class="|class=\\\')(\\w+)?(\\s)?' + cssClass);
    ...
    function hasClass(markup, cssClass) {
        var regex = new RegExp( 'class=["\']([\\w\\s]+)?' + cssClass + '["\'\\s]' );
    ...
    // SHORTEST WAY TO CODE:
    function hasClass(markup, cssClass) {
        var regex = new RegExp( 'class=["\'].*' + cssClass + '.*["\']' );
        return regex.test(markup);
    }
    // BUT THE MORE SPECIFIC THE REGEX IS, THE FASTER THE CODE WILL RUN (IT WILL TRY LESS OPTIONS OF CHARACTERS). SO THE SHORTEST CODE WON`T BE NECESSARILY THE FASTEST RUN.
        
    var classes = [ 'container', 'text', 'date', 'excerpt', 'main' ];
    classes.forEach(function(myClassesArray) {
        console.log( hasClass( markup, myClassesArray ) + ' para a classe ' + myClassesArray);
    });
    // *************************************************************    
